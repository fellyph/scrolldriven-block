...
function my_scroll_block_register_assets() {
    ...
    // Frontend behavior.
    $view_asset_path = $dir . 'build/view.asset.php';
    if ( file_exists( $view_asset_path ) ) {
        $view_asset = include $view_asset_path;
        wp_register_script(
            'my-scroll-block-view',
            plugins_url( 'build/view.js', __FILE__ ),
            isset( $view_asset['dependencies'] ) ? $view_asset['dependencies'] : array(),
            isset( $view_asset['version'] ) ? $view_asset['version'] : filemtime( $dir . 'build/view.js' ),
            true
        );
    }
}
...
add_action( 'enqueue_block_editor_assets', function() {
    ...
    if ( wp_script_is( 'my-scroll-block-view', 'registered' ) ) {
        // Also load the view script in editor so previews work.
        wp_enqueue_script( 'my-scroll-block-view' );
    }
} );
...
add_filter( 'render_block', function( $block_content, $block ) {
    ...
    if ( isset( $attrs['animationType'] ) && 'none' !== $attrs['animationType'] ) {
        ...
        if ( is_string( $block_content ) && $block_content !== '' && strpos( $block_content, 'scroll-anim-block' ) === false ) {
            ...
            $animation_duration = isset( $attrs['animationDuration'] ) ? (int) $attrs['animationDuration'] : 500;

            $add_classes = sprintf( 'scroll-anim-block scroll-anim-%s', strtolower( str_replace( ' ', '-', $animation_type ) ) );
            $style_vars  = sprintf( '--anim-delay:%dms;--anim-duration:%dms;', $animation_delay, $animation_duration );
